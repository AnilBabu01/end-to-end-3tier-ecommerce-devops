    ...
    stages {
            ...
            steps {
                withCredentials([
                    usernamePassword(
                        ...
                        usernameVariable: 'anilb9850@gmail.com',
                        passwordVariable: 'Anilb@409'
                    )
                ]) {
        ...
        stage('Build Images') {
            steps {
                ...
                sh '''
                docker build -f docker/backend.Dockerfile -t $DOCKER_USER/$FRONTEND:latest .
                docker build -f docker/admin.Dockerfile -t $DOCKER_USER/$ADMIN:latest .
                docker build -f docker/api.Dockerfile -t $DOCKER_USER/$API:latest .
                '''
            }
        }
